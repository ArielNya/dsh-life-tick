...
export class LifeTickRuntime {
  ...
  onAssistantText(text) {
    ...
    this.awaitingReply = false
    if (isNoPing(text)) {
      if (isVisibleKind(this.lastKind) && this.missedCount > 0) this.missedCount -= 1
  ...
  deliverTick(config, kind) {
    ...
    if (pending !== undefined) {
      ...
      this.lastKind = kind
      this.logger?.info?.(`life-tick: replaced queued ${kind} for agent "${this.agent.id}"`)
      return
    }
    ...
    this.lastKind = kind
    if (isVisibleKind(kind)) this.missedCount += 1
