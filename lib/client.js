  ...
  factory: (require) => {
    ...
    function makeSection(controller) {
      return function HeartbeatSection() {
        return React.createElement(SectionBoundary, null, React.createElement(HeartbeatForm, { controller }))
      }
    }

    function HeartbeatForm({ controller }) {
        ...
        React.createElement('div', { style: rowStyle },
          ...
          React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '6px' } },
            React.createElement('input', {
              ...
              max: 100,
              disabled: loading || disabled,
              value: loading ? '' : (value?.pauseAfterMissed ?? 3),
              onChange: (event) => {
                const beats = Number(event.target.value)
                if (Number.isFinite(beats) && beats >= 0 && beats <= 100) {
                  controller.set('pauseAfterMissed', Math.round(beats))
                }
              },
              style: inputStyle,
            }),
    ...
    function apply(ctx) {
      ...
      try {
        ...
        controller = settingsScope.bind({ namespace: 'heartbeat' })
      } catch (error) {
